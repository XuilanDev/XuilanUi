...
local function initElementState(el)
	...
	local function renderToggle(section, opt, elementRef)
		...
		if opt.Bind then
			RegisterBind(opt.Bind, function()
...
function section:AddNotification(n)
	...
	--==================================================
	-- LEGO-style helpers (optional)
	-- Allows: Window:AddSection(...) then Window:AddToggle(...) etc.
	-- Does NOT change existing Section:Add... API.
	--==================================================
	local function requireCurrentSection()
		if not Window._currentSection then
			error("No active section. Call Window:AddSection({...}) before adding elements.", 2)
		end
		return Window._currentSection
	end

	function Window:AddToggle(opt)      return requireCurrentSection():AddToggle(opt) end
	function Window:AddButton(opt)      return requireCurrentSection():AddButton(opt) end
	function Window:AddSlider(opt)      return requireCurrentSection():AddSlider(opt) end
	function Window:AddDropdown(opt)    return requireCurrentSection():AddDropdown(opt) end
	function Window:AddTextBox(opt)     return requireCurrentSection():AddTextBox(opt) end
	function Window:AddLabel(opt)       return requireCurrentSection():AddLabel(opt) end
	function Window:AddHeadText(opt)    return requireCurrentSection():AddHeadText(opt) end
	function Window:AddColorPicker(opt) return requireCurrentSection():AddColorPicker(opt) end
	function Window:AddNotification(opt) return requireCurrentSection():AddNotification(opt) end
	-- NOTE: Window:AddSeparator() is reserved for LEFT LIST separator as you designed.
	-- For in-section separator keep using: Section:AddSeparator()
