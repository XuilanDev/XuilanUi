...
function XuilanLib:CreateWindow(settings)
	...
	--==================================================
	-- Resize handle (outside bottom-right)
	--==================================================
	...
	resizeHandle.Position = UDim2.new(1, 4, 1, 4) -- slightly outside, close
	resizeHandle.ZIndex = 2000
	...
	-- simple diagonal glyph
	local glyph1 = Instance.new("Frame")
	glyph1.BackgroundColor3 = Color3.fromRGB(255,255,255)
	glyph1.BackgroundTransparency = 0
	glyph1.BorderSizePixel = 0
	glyph1.Size = UDim2.new(0, 16, 0, 3)
	glyph1.Position = UDim2.new(0, 2, 0, 10)
	glyph1.Rotation = -45
	glyph1.ZIndex = 2001
	glyph1.Parent = resizeHandle

	local glyph2 = Instance.new("Frame")
	glyph2.BackgroundColor3 = Color3.fromRGB(255,255,255)
	glyph2.BackgroundTransparency = 0
	glyph2.BorderSizePixel = 0
	glyph2.Size = UDim2.new(0, 12, 0, 3)
	glyph2.Position = UDim2.new(0, 6, 0, 12)
	glyph2.Rotation = -45
	glyph2.ZIndex = 2001
	glyph2.Parent = resizeHandle

	local rg1 = Instance.new("UICorner")
	rg1.CornerRadius = UDim.new(1,0)
	rg1.Parent = glyph1
	local rg2 = Instance.new("UICorner")
	rg2.CornerRadius = UDim.new(1,0)
	rg2.Parent = glyph2
	...
	UserInputService.InputChanged:Connect(function(input)
		...
		-- Keep top-left corner stable while resizing from bottom-right
		local dW = newW - startSize.X.Offset
		local dH = newH - startSize.Y.Offset
		mainFrame.Size = UDim2.new(0, newW, 0, newH)
		mainFrame.Position = UDim2.new(
			startPos.X.Scale, startPos.X.Offset + (dW/2),
	...
	-- Content fade group (for fast section transitions)
	...
	contentGroup.ZIndex = 1010
	contentGroup.GroupTransparency = 0
	contentGroup.Parent = innerMask
	...
	contentScroll.Parent = contentGroup
...
local function initElementState(el)
	...
	local function renderHeadText(section, opt)
		...
		head.Text = opt.Name or opt.Text or "Head Text"
		...
		head.Position = UDim2.new(0,10,0,0)
		head.Size = UDim2.new(1,-20,1,0)
		head.ZIndex = 1013

		local align = tostring(opt.Alignment or opt.Align or "mid"):lower()
		if align == "left" then
			head.TextXAlignment = Enum.TextXAlignment.Left
		elseif align == "right" then
			head.TextXAlignment = Enum.TextXAlignment.Right
		else
		...
		table.insert(ThemeTargets.Rows, {Type="HeadText", Label=head})
	end
	...
	local function setSelected(idx)
		...
		-- Fade out current page fast
		local function fadeTo(val)
			if not contentGroup then return end
			local tw = TweenService:Create(contentGroup, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {GroupTransparency = val})
			tw:Play()
			return tw
		end
		...
		local function renderNow()
			...
			-- Fade in new page fast
			if contentGroup then
				contentGroup.GroupTransparency = 1
				local inTw = TweenService:Create(contentGroup, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {GroupTransparency = 0})
				inTw:Play()
			end

			-- Small bounce for rows
			task.defer(function()
		...
		if outTw then
			outTw.Completed:Once(renderNow)
		else
			renderNow()
		end
	end
