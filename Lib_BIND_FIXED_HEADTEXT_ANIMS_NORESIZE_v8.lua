...
for _, b in ipairs(ThemeTargets.SectionButtons) do
	...
	local function applySectionSelected(btn, selected)
		...
		-- Fade the selected section background quickly
		local targetT = selected and INNER_T or 1
		local tw = TweenService:Create(btn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = targetT})
		tw:Play()
	end
...
local function initElementState(el)
	...
	local function renderDropdown(section, opt, elementRef)
		...
		selectBtn.Font = ParseFont(opt.ValueFont or "SourceSansBold")
		...
		-- caret indicator (down/up)
		local caret = Instance.new("TextLabel")
		caret.Name = "Caret"
		caret.BackgroundTransparency = 1
		caret.BorderSizePixel = 0
		caret.Text = "˅"
		caret.Font = ParseFont(opt.ValueFont or "SourceSansBold")
		caret.TextSize = clampTextSize(opt.ValueTextSize, 14)
		caret.TextColor3 = Color3.fromRGB(255,255,255)
		caret.TextXAlignment = Enum.TextXAlignment.Center
		caret.TextYAlignment = Enum.TextYAlignment.Center
		...
		local function openMenu()
			...
			for i,optVal in ipairs(options) do
				...
				b.Text = s
				b.Font = ParseFont(opt.ValueFont or "SourceSansBold")
		...
		local function renderNow()
			...
			if contentGroup then
				...
				inTw:Play()
						...
						-- Small pop-up for rows (no layout movement)
			task.defer(function()
				for _, child in ipairs(contentScroll:GetChildren()) do
					if child:IsA("Frame") and child ~= bottomSpacer then
						local p = child.Position
						child.Position = UDim2.new(p.X.Scale, p.X.Offset, p.Y.Scale, p.Y.Offset + 6)
						TweenService:Create(child, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = p}):Play()
					end
				end
			end)
